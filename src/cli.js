...
import {
  ...
  isConnected,
  Prim,
} from "./algorithm/graphRealization.js";
    ...
    try {
      switch (command) {
        ...
        case "load": {
            ...
            type: () =>
              clack.select({
                ...
                options: [
                  ...
                  {
                    value: "undirected",
                    label: "Неориентированный (undirected)",
                  },
                ],
          ...
          clack.log.success(
            `Граф загружен из ${path.basename(result.filePath)} (тип: ${result.type}).`,
          );
          break;
        }
        case "save": {
          if (!currentGraph)
            throw new Error(
              "Граф не инициализирован. Используйте init или load.",
            );
          ...
          const result = await clack.group({
            ...
            weight: () =>
              clack.text({
                ...
                initialValue: "1",
                validate: (value) =>
                  !isNaN(Number(value)) || "Должно быть числом",
              }),
          });

          if (clack.isCancel(result)) break;

          currentGraph.add_edge(
            Number(result.from),
            Number(result.to),
            Number(result.weight),
          );
          clack.log.success(
            `Ребро ${result.from} -> ${result.to} (вес ${result.weight}) добавлено.`,
          );
          break;
        }
        case "remove-vertex": {
        ...
        case "vertices": {
          if (!currentGraph) throw new Error("Граф не инициализирован.");
          const verts = Array.from(currentGraph.filedata.keys()).sort(
            (a, b) => a - b,
          );
